...
#input_csv = read.table(input_observation, sep=",", dec = ".", header = TRUE, stringsAsFactors = FALSE) %>%
#  mutate(time=as.POSIXct(time, tz = "GMT", format="%Y-%m-%d %H:%M:%S")) %>%
#  select(time, "USH9.snow.depth..mm.") %>%
#  filter(as.Date(time) >= as.Date("1994-10-04") &
#           as.Date(time) <  as.Date("2014-10-04"))
...
# scatterplot
###################
png(filename = paste0("C:/Users/lea/Documents/Uni/Innsbruck/2026 SS/Gebirgsforschung/Outputdata/Plots/scatterplot_sap_", model_run, ".png"),
    width = 1000, height = 600, res = 150)  # Breite, Höhe in Pixel, Auflösung
...
par(pty = "s",
    ...
    cex.axis = 0.8,    # Achsen
    cex.lab = 0.85,    # Achsenbeschriftung
    cex.main = 0.9)    # Titel
plot(ObservationData$SD_mm, SimulationData$SD_mm, type = "p", col = "black", ann = FALSE, pch=19, cex=0.6)
