...
png(filename = paste0("C:/Users/lea/Documents/Uni/Innsbruck/2026 SS/Gebirgsforschung/Outputdata/Plots/lineplot_sap_meter_", model_run, ".png"),
...
# 1 - observation
y_lim = range(c(ObservationData$SD_m, SimulationData$SD_m), na.rm = TRUE)

plot(ObservationData$DateTime, ObservationData$SD_m, type='l', col='red', ann=FALSE, ylim = y_lim)
...
# 2 - simulation
lines(SimulationData$DateTime, SimulationData$SD_m, type = 'l', col='blue', lty='twodash')

legend("topright", legend = c("Beobachtung", "Simulation"), col = c("red", "blue"), lty = c(1,2), bty = "n")

summary(ObservationData$SD_m)
...
png(filename = paste0("C:/Users/lea/Documents/Uni/Innsbruck/2026 SS/Gebirgsforschung/Outputdata/Plots/scatterplot_sap_version3_meter_", model_run, ".png"),
    width = 1000, height = 600, res = 150)  # Breite, Höhe in Pixel, Auflösung
...
par(pty = "s",
    ...
    cex.main = 1.2)    # Titel
plot(ObservationData$SD_m, SimulationData$SD_m, type = "p", col = "darkblue", ann = FALSE, pch=19, cex=0.5)
...
# Lineare Regression
fit_model = lm(SimulationData$SD_m ~ ObservationData$SD_m) # hier zuerst die y werte, dann die x werte; mit tilde ~ trennen
...
Nash=round(NSE(SimulationData$SD_m, ObservationData$SD_m),2) # Nash, damit das erste NSE nicht gleich heißt und nicht verwechselt wird
PBIAS=round(pbias(SimulationData$SD_m, ObservationData$SD_m),2)
